            ...
            <div style="display: flex; flex-direction: column; height: 100%; max-width: 1400px; margin: 0 auto;">
                ...
                <div style="background: white; border-radius: 20px; box-shadow: 0 8px 30px rgba(0,0,0,0.08); flex: 1; display: flex; flex-direction: column; overflow: hidden; border: 1px solid #dee2e6;">
                    ...
                    <div style="flex: 1; padding: 1.5rem 2rem; overflow: hidden; display: flex; flex-direction: column;">
                        ...
                        @if($errors->any())
                            <div style="background: linear-gradient(135deg, #fff5f5 0%, #ffe5e5 100%); border-left: 5px solid #dc3545; color: #721c24; padding: 0.75rem 1rem; border-radius: 10px; margin-bottom: 1rem; font-size: 0.85rem;">
                                <strong style="display: block; margin-bottom: 8px;">Terdapat kesalahan input</strong>
                                                ...
                                                @foreach($kategoris as $kategori)
                                                    <label style="display: flex; align-items: center; gap: 6px; padding: 6px 12px; border: 2px solid #e9ecef; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 500;">
                                                        <input type="checkbox"
                                                               name="kategori_ids[]"
                                                               value="{{ $kategori->id }}"
                                                               {{ in_array($kategori->id, old('kategori_ids', $buku->kategoris->pluck('id')->toArray())) ? 'checked' : '' }}
...
<script>
    ...
    document.getElementById('formBuku').addEventListener('submit', function(e) {
        ...
        if (!isValid) {
            ...
            if (emptyFields.length > 0) emptyFields[0].focus();
        }
    });
</script>
@endsection
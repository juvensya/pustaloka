        ...
        <div class="col p-4" style="background:#f8f9fa;min-height:100vh;">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem;margin-bottom:1.5rem;">
                <div style="display:flex;align-items:center;gap:12px;">
                    ...
                    <div>
                        <h2 style="font-size:1.75rem;font-weight:800;color:#141516;margin:0;letter-spacing:-0.5px;">Data Pengembalian</h2>
                        <p style="color:#666;font-size:0.875rem;margin:0;">Total <strong>{{ $data->total() }}</strong> pengembalian</p>
                    </div>
                </div>
                <form action="{{ route('admin.peminjaman.pengembalian') }}" method="GET">
                    <div style="position:relative;">
                        <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari nama peminjam atau judul buku..."
                            style="padding:10px 44px 10px 16px;border:2px solid #e9ecef;border-radius:10px;font-size:0.875rem;outline:none;width:300px;"
            ...
            @if($search)
                <div style="margin-bottom:1rem;font-size:0.85rem;color:#666;">
                    ...
                    <a href="{{ route('admin.peminjaman.pengembalian') }}" style="color:#8B0000;font-weight:600;text-decoration:none;">✕ Hapus filter</a>
                </div>
            @endif
                    ...
                    <table style="width:100%;border-collapse:collapse;">
                        <thead>
                            <tr style="background:#f8f9fa;border-bottom:2px solid #e9ecef;">
                                @foreach(['No','Peminjam','Buku','Tgl Pinjam','Tgl Kembali','Tgl Dikembalikan','Info',''] as $h)
                        ...
                        <tbody>
                            ...
                            <tr style="border-bottom:1px solid #f5f5f5;background:white;"
                                ...
                                <!-- Buku -->
                                <td style="padding:1.1rem 1.5rem;vertical-align:middle;">
                                    <div style="font-size:1rem;font-weight:600;color:#2c3e50;">{{ $pinjam->buku->judul }}</div>
                                    <div style="font-size:0.9rem;color:#9e6060;">{{ $pinjam->buku->penerbit ?? '-' }}</div>
                                </td>

                                <td style="padding:1.1rem 1.5rem;font-size:0.95rem;color:#666;vertical-align:middle;white-space:nowrap;">{{ $pinjam->tanggal_pinjam ?? '-' }}</td>
                                <td style="padding:1.1rem 1.5rem;font-size:0.95rem;color:#666;vertical-align:middle;white-space:nowrap;">{{ $pinjam->tanggal_kembali ?? '-' }}</td>
                                <td style="padding:1.1rem 1.5rem;font-size:0.95rem;color:#666;vertical-align:middle;white-space:nowrap;">{{ $pinjam->tanggal_dikembalikan ?? '-' }}</td>

                                <!-- Info -->
                                <td style="padding:1.1rem 1.5rem;vertical-align:middle;">
                                    @if($terlambatHari > 0)
                                        <span style="display:inline-flex;align-items:center;gap:0.3rem;background:#fef2f2;color:#8B0000;padding:4px 11px;border-radius:999px;font-size:0.9rem;font-weight:700;">
                                            <span style="width:6px;height:6px;border-radius:50%;background:currentColor;display:inline-block;"></span>Terlambat {{ $terlambatHari }} hari
                                        </span>
                                    @else
                                        <span style="display:inline-flex;align-items:center;gap:0.3rem;background:#f0fdf4;color:#15803d;padding:4px 11px;border-radius:999px;font-size:0.9rem;font-weight:700;">
                                            <span style="width:6px;height:6px;border-radius:50%;background:currentColor;display:inline-block;"></span>Tepat Waktu
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td style="padding:1.1rem 1.5rem;text-align:center;vertical-align:middle;position:relative;">
                                    <button onclick="toggleDD(event,'dd{{$pinjam->id}}')"
                                    ...
                                    <div id="dd{{$pinjam->id}}" style="display:none;position:fixed;background:white;border:1px solid #e9ecef;border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,0.13);min-width:160px;z-index:9999;overflow:hidden;">
                                        <form action="{{ route('admin.peminjaman.destroy',$pinjam->id) }}" method="POST" onsubmit="return confirm('Hapus data ini?')">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                            <tr>
                                <td colspan="8" style="padding:60px;text-align:center;color:#9e6060;font-size:1.02rem;">
                                    {{ $search ? '🔍 Tidak ada hasil untuk "'.$search.'"' : '📭 Belum ada data pengembalian' }}
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
